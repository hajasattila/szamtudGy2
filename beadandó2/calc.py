...
# Egylistában el van tárolva az összes token, amit használni fogunk
tokens = [
...
def t_FLOAT(t):
    ...
    try:
        ...
        print("Valahogy még a float sem jó %d", t.value)
        t.value = 0
    return t
...
def t_INT(t):
    ...
    try:
        ...
        print("Túl nagy az int értéked %d", t.value)
        t.value = 0
    return t
...
#skippelem a jelenlegi tokent, és visszaadja a hibát. Ez is egy speciális ply function
def t_error(t):
...
#definiáljuk a nyelvtanunkat. amit engedélyezünk: expressions, var_assigns, és üres mezők
def p_calc(p):
    '''
    calc : expression
         | var_assign
...
def p_error(p):
    print("Színtakszis HIBA!")
    if not isinstance(t_NAME, str):
        print("Nem kezdődhet betűvel a változód!")
